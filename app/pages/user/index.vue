...
const { data: eventsResp, pending, error } = await useAsyncData("events-user", () => $fetch('/api/events'));
...
<template>
  <div class="relative w-full h-full overflow-x-hidden">
    <div class="flex justify-center items-center flex-col gap-2">
      ...
      <Button variant="secondary" class="text-lg w-40" @click="toggleVisibility">Generar QR</Button>
          ...
          <template v-if="events.length === 0">
            <div class="w-full flex justify-center mt-8">
              <Card class="bg-transparent border-transparent shadow-none md:w-2/3 w-full p-8 flex flex-col items-center text-center">
                ...
                <p class="text-muted-foreground -mt-2.5 mb-6">Por ahora no hay eventos en curso. Vuelve más tarde.</p>
              </Card>
            </div>
          </template>
          <template v-else>

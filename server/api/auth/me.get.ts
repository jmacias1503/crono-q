  ...
  try {
    ...
    if (payload.userType === 'student' && payload.student_id) {
      const student = await prisma.students.findUnique({ 
        where: { student_id: Number(payload.student_id) }, 
        select: { student_id: true, first_name: true, last_name: true, career: true, semester: true } 
      });
    ...
    if (payload.userType === 'admin' && payload.admin_id) {
      const admin = await prisma.admins.findUnique({ 
        where: { admin_id: Number(payload.admin_id) }, 
